...
static int LSM6DS0_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	/* flat_power will be used when output data. */
	flag_power = sensor_power;
	if(flag_power)
		flag_i = 0;
...
static int LSM6DS0_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		{	
			flag_i++;
			obj->data[LSM6DS0_AXIS_X] = 0;
			obj->data[LSM6DS0_AXIS_Y] = 0;
			obj->data[LSM6DS0_AXIS_Z] = 0;
			data[LSM6DS0_AXIS_X] = 0;
			data[LSM6DS0_AXIS_Y] = 0;
			data[LSM6DS0_AXIS_Z] = 0;
		}
		else
		{
			flag_i = 0;
...
#ifdef CONFIG_COMPAT
static long LSM6DS0_Gyro_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
	...
	{
		GYRO_ERR("file->f_op or file->f_op->unlocked_ioctl is NULL\n");
		return -ENOTTY;
	}
	...
	switch(cmd)
	{
		case COMPAT_GYROSCOPE_IOCTL_INIT:
			err = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_INIT, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_INIT is failed!\n");
			}
			break;
		...
		case COMPAT_GYROSCOPE_IOCTL_SMT_DATA:
			...
			err = file->f_op->unlocked_ioctl(file,GYROSCOPE_IOCTL_SMT_DATA, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_SMT_DATA is failed!\n");
			}				 
			break;	  
		...
		case COMPAT_GYROSCOPE_IOCTL_READ_SENSORDATA:
			...
			err = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_READ_SENSORDATA, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_READ_SENSORDATA is failed!\n");
			}				 
			break;
		...
		case COMPAT_GYROSCOPE_IOCTL_SET_CALI:
			...
			err = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_SET_CALI, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_SET_CALI is failed!\n");
			}				 
			break;

		case COMPAT_GYROSCOPE_IOCTL_CLR_CALI:
			err = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_CLR_CALI, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_CLR_CALI is failed!\n");
			}
		...
		case COMPAT_GYROSCOPE_IOCTL_GET_CALI:
			...
			err = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_GET_CALI, (unsigned long)arg64);
			if(err < 0)
			{
				GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_GET_CALI is failed!\n");
			}		
			break;
...
#ifdef CONFIG_COMPAT
    .compat_ioctl = LSM6DS0_Gyro_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
...
{
    //hulimin 
    //unsigned long m1_addr = (*(volatile unsigned long *)(client->addr));
	printk(KERN_EMERG"limin line =%d fun= %s m1_addr =%x\n",__LINE__,__func__,client->addr);
    //hulimin
    client->addr =0xd4>>1;
	...
	databuf[1] = 0x40;                                                                                                                                                           
	databuf[0] = LSM6DS0_CTL_REG3;                                                                                                                                               
                                                                                                                                                                   
	res = i2c_master_send(client, databuf, 0x2);                                                                                                                                 
	if(res <= 0)
	{
		printk("Demon gyro write LSM6DS0_CTL_REG3 err\n");
	}
	databuf[1] = 0x00;                                                                                                                                                           
	databuf[0] = LSM6DS0_CTL_REG2;                                                                                                                                               
                                                                                                                                                                   
	res = i2c_master_send(client, databuf, 0x2);                                                                                                                                 
	if(res <= 0)
	{
		printk("Demon gyro write LSM6DS0_CTL_REG2 err\n");
	}
