...
static void hdmi_drv_get_params(HDMI_PARAMS *params)
{
    ...
    if(MHL_Connect_type== MHL_3D_GLASSES)
        params->cabletype                               = MHL_3D_GLASSES;
    ...
    if(ptr)
    {
		...
		if(ptr->ui4_pal_resolution == 0)
		{
	        ...
	        if(si_dev_context && packed_pixel_available(si_dev_context))
                ...
                /* Add only 1080p30 support for LETV 3D GLASSES */
                if (MHL_Connect_type == MHL_3D_GLASSES) {
                        ptr->ui4_pal_resolution = SINK_1080P60|SINK_1080P30|SINK_720P60;
                        HDMI_LOG("MHL edid resolution reset to LETV_3D_GLASSED\n");
                }
    }
