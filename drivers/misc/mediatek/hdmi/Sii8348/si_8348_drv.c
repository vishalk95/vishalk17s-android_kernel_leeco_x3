...
//	hw_context->outgoingAviPayLoad  = hw_context->current_avi_info_frame.payLoad.hwPayLoad;	// TODO: FD, TBC, should be ok?
	...
	if(VCDB_found)
	{
		...
		TX_DEBUG_PRINT(("VCDB_found = true, limited range\n"));
	}
	else
	{
		hw_context->outgoingAviPayLoad.namedIfData.ifData_u.infoFrameData[2] = 0x04;  
		TX_DEBUG_PRINT(("VCDB_found= false. defult range\n"));
	}
...
static void unmute_video(struct drv_hw_context *hw_context)
{
	...
	//if(si_edid_sink_is_hdmi(hw_context->intr_info->edid_parser_context))
	{
		...
		if(VCDB_found)
		{
			mhl_tx_modify_reg(hw_context, REG_TPI_OUTPUT, BIT_TPI_OUTPUT_QUAN_RANGE_MASK, 					BIT_TPI_OUTPUT_QUAN_RANGE_LIMITED);
			mhl_tx_modify_reg(hw_context, REG_VID_ACEN, 0x02, 0x02);

			TX_DEBUG_PRINT(("VCDB_found = true, limited range\n"));
		}
		else
		{
			mhl_tx_modify_reg(hw_context, REG_TPI_OUTPUT, BIT_TPI_OUTPUT_QUAN_RANGE_MASK, 					BIT_TPI_OUTPUT_QUAN_RANGE_FULL);
			mhl_tx_modify_reg(hw_context, REG_VID_ACEN, 0x02, 0x00);
			TX_DEBUG_PRINT(("VCDB_found= false. defult range\n"));
		}
	}
	#if 0
...
//letv add for detect mhl cable detection
int mhl_connect = 0;
static int int_4_isr(struct drv_hw_context *hw_context, uint8_t int_4_status)
{
