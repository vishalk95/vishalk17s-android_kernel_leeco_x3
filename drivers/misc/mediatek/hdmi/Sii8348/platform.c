...
extern int bq2589x_dpdm_enable(void);
#endif
...
#define USE_DELAY_FOR_INIT_INTR 1

#ifdef USE_DELAY_FOR_INIT_INTR
struct timer_list mhl_interrupt_timer;
static void mhl_interrupt_enable_cb(unsigned long n)
{
	printk("warning, USE_DELAY_FOR_INIT_INTR, %s, enter\n", __func__);
	Unmask_MHL_Intr();
	del_timer(&mhl_interrupt_timer);
	printk("warning, USE_DELAY_FOR_INIT_INTR, %s, exit\n", __func__);
}

int mhl_interrupt_enable_delay()
{
	int ms_latency = 3000;  /// 3s
	memset((void *)&mhl_interrupt_timer, 0, sizeof(mhl_interrupt_timer));
	mhl_interrupt_timer.expires = jiffies + msecs_to_jiffies(ms_latency);
	mhl_interrupt_timer.function = mhl_interrupt_enable_cb;
	init_timer(&mhl_interrupt_timer);
	add_timer(&mhl_interrupt_timer);
	printk("warning, USE_DELAY_FOR_INIT_INTR, %s\n", __func__);
	return 0;
}
#endif
