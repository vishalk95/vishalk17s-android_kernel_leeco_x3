...
#include "s5k3l9mipiraw_Sensor.h"
#include "s5k3l9otp.h"
...
//#define FANPENGTAO
#define PFX "S5K3L9"
#define LOG_1 LOG_INF("S5K3L9,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 100,//85,
	...
	.slim_video = {
        .pclk = 560000000,    //record different mode's pclk
        .linelength  = 5808,    //record different mode's linelength
        .framelength = 1607,   //record different mode's framelength
        .startx = 0,     //record different mode's startx of grabwindow
        .starty = 0,     //record different mode's starty of grabwindow
        .grabwindow_width  = 2104,  //record different mode's width of grabwindow
  ...
  .custom2 = {
		...
		/*	 following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario	*/
		.mipi_data_lp2hs_settle_dc = 100,//85,//unit , ns
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,	
	},
  .custom3 = {
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =	 
{
 ...
 { 4208, 3120,	184,  480, 3840, 2160, 1392,  784,   0,	0, 1392,  784, 	 0, 0, 1392,  784}, //hight speed video,20150828,720p
 { 4208, 3120,	184,  480, 3840, 2160, 2104,  1184,   0,	0, 2104,  1184, 	 0, 0, 2104,  1184},// slim video ,1080p
...
//ihdr_write_shutter_gain not support for s5k3l9
...
static void sensor_init(void)
{
  LOG_INF("E\n");
	...
	write_cmos_sensor(0x37CC, 0x0001);	//20150916 update by SS HQ
	...
	write_cmos_sensor(0xF460, 0x0030);	//20150916 update by SS HQ
	write_cmos_sensor(0xF414, 0x24C2);
	write_cmos_sensor(0xF416, 0x0183);
	write_cmos_sensor(0xF468, 0x0405);
	write_cmos_sensor(0x3424, 0x0A07);	//20150916 update by SS HQ
	...
	write_cmos_sensor(0x3058, 0x0100);	//20150916 update by SS HQ
	write_cmos_sensor(0x6028, 0x2000);
	write_cmos_sensor(0x602A, 0x108A);
	write_cmos_sensor(0x6F12, 0x0359);
	write_cmos_sensor(0x6F12, 0x0100);  
	write_cmos_sensor(0x6028, 0x4000);	//20150916 update by SS HQ
	write_cmos_sensor(0x1124, 0x4100);	//20150916 update by SS HQ
	write_cmos_sensor(0x1126, 0x0000);	//20150916 update by SS HQ
	write_cmos_sensor(0x112C, 0x4100);	//20150916 update by SS HQ
	write_cmos_sensor(0x112E, 0x0000); 	//20150916 update by SS HQ
...
static void hs_video_setting()
	{
		LOG_INF("E\n");
	  //$MV1[MCLK:24,Width:1392,Height:784,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
	  write_cmos_sensor(0x0100, 0x0000);
	  write_cmos_sensor(0x6028, 0x2000);
	  write_cmos_sensor(0x602A, 0x0F74);
	  write_cmos_sensor(0x6F12, 0x0040);
...
static void hs_video_setting720P()
	{
		LOG_INF("E\n");
...
static void slim_video_setting1080P()
	{
		LOG_INF("E\n");
	  //$MV1[MCLK:24,Width:1920,Height:1080,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
	  write_cmos_sensor(0x0100, 0x0000);
	  write_cmos_sensor(0x6028, 0x2000);
	  write_cmos_sensor(0x602A, 0x0F74);
	  write_cmos_sensor(0x6F12, 0x0040);
	  ...
	  write_cmos_sensor(0x0308, 0x0008);
	  write_cmos_sensor(0x0342, 0x16B0);
	  write_cmos_sensor(0x0340, 0x0647);
	  write_cmos_sensor(0x0202, 0x0200);
	  write_cmos_sensor(0x0200, 0x00C6);
	  write_cmos_sensor(0x0B04, 0x0101);
	  write_cmos_sensor(0x0B08, 0x0000);
 ...
 static void slim_video_setting()
	{
		LOG_INF("E\n");
	  //$MV1[MCLK:24,Width:2104,Height:1184,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
	  write_cmos_sensor(0x0100, 0x0000);
	  write_cmos_sensor(0x6028, 0x2000);
	  write_cmos_sensor(0x602A, 0x0F74);
	  write_cmos_sensor(0x6F12, 0x0040);
	  ...
	  write_cmos_sensor(0x034C, 0x0838);
	  write_cmos_sensor(0x034E, 0x04A0);
	  write_cmos_sensor(0x0900, 0x0112);
	  write_cmos_sensor(0x0380, 0x0001);
	  write_cmos_sensor(0x0382, 0x0001);
	  write_cmos_sensor(0x0384, 0x0001);
...
/*
//sensor info as below,
.pclk = 560000000, //record different mode's pclk
.linelength = 9392, //record different mode's linelength
.framelength = 59625, //record different mode's framelength
.startx = 0, //record different mode's startx of grabwindow
.starty = 0, //record different mode's starty of grabwindow
.grabwindow_width = 4208, //record different mode's width of grabwindow
.grabwindow_height = 3120, //record different mode's height of grabwindow
...
static void LongExp_setting(void)
{
  LOG_INF("E\n");
  ...
  write_cmos_sensor(0x0340, 0xe8e9);
  write_cmos_sensor(0x0202, 0xe8e0);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
  write_cmos_sensor(0x0B00, 0x0007);
...
.max_framerate = 10, 
*/	
		imgsensor.pclk = 560000000;
		imgsensor.line_length = 9392;
		imgsensor.frame_length = 59625;
...
extern void read_s5k3l9_static_otp();
extern kal_bool BU6429_Alive(void);
extern int Main_Camera_MID;// 

extern int LC898217AF_Init_Thread(void *unused);
extern struct task_struct  *lc898217af_init_thread;
extern int g_s4LC898217AF_Inited;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(BU6429_Alive() == FALSE)
				{
					read_s5k3l9_static_otp();
					Main_Camera_MID = 3;
					LOG_INF("3L9 SUMSUNG MODULE\n");
					*sensor_id = S5K3L9_SENSOR_ID;
					return ERROR_NONE;
				}
				else
				{
					LOG_INF("3L9 NOT SUMSUNG MODULE\n");	
...
static kal_uint32 open(void)
{
	...
	lc898217af_init_thread= kthread_run(LC898217AF_Init_Thread, 0, "LC898217AF_Init"); 
...
LOG_INF("[FANPENGTAO] TEST THE PIP 15FPS SETTING!\n");
#endif
...
static char *hwid_string[] = {"EVT", "DVT1", "DVT2", "PVT"} ;
static unsigned revision_gpio[] = {53,54};

static int target_get_board_hwid_no(void)
{
    int revision = 0 , i = 0;
	
    for(i = 0 ;i < ARRAY_SIZE(revision_gpio) ; i++)
    {
        if(mt_get_gpio_in(revision_gpio[i]))
        {
            revision += (1 << i);
        }
    }
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//hwid_num = target_get_board_hwid_no();
	hwid_num = get_3l9_dvt_id();
	...
	else
	{
		if(hwid_num < 2)
		{
			//sensor_info->PDAF_Support = 0x0F;
			sensor_info->PDAF_Support = 0;
			LOG_INF("EVT DVT1 disable Samsung PDAF\n");
		}
		else
		{
			sensor_info->PDAF_Support = 1;
			LOG_INF("DVT2 PVT enable Samsung PDAF\n");
		}
	}
	#endif
...
STEP1_VERIFY_INPUT_WIN_W=3;
STEP1_VERIFY_INPUT_WIN_H=3;
STEP1_VERIFY_INPUT_MIN=60;
STEP1_VERIFY_INPUT_MAX=230;
STEP1_VERIFY_INPUT_CORNER_MIN=40;
STEP1_VERIFY_INPUT_CORNER_MAX=230;
STEP1_VERIFY_OUTPUT_DIFF_MIN=40;
STEP1_VERIFY_OUTPUT_DIFF_MAX=250;
STEP2_VERIFY_INPUT_WIN_W=2;
STEP2_VERIFY_INPUT_WIN_H=2;
STEP2_VERIFY_INPUT_MIN=120;
STEP2_VERIFY_INPUT_MAX=230;
STEP2_VERIFY_INPUT_IMAGE_TILT_MAX=50;
STEP2_VERIFY_OUTPUT_LINEARITY_MIN=96;
STEP2_VERIFY_OUTPUT_LINEARITY_AVG=99;
STEP2_VERIFY_OUTPUT_DISPARITY_MIN=5;
STEP2_VERIFY_OUTPUT_DISPARITY_MAX=80;
STEP2_VERIFY_OUTPUT_DYNAMIC_RANGE=20;
STEP2_VERIFY_OUTPUT_PD_RANGE=8.5;
STEP3_VERIFY_INPUT_WIN_W=4;
STEP3_VERIFY_INPUT_WIN_H=4;
STEP3_VERIFY_INPUT_MIN=120;
STEP3_VERIFY_INPUT_MAX=230;
STEP3_VERIFY_OUTPUT_LINEARITY_MIN=96;
STEP3_VERIFY_OUTPUT_DISPARITY_MIN=5;
STEP3_VERIFY_OUTPUT_DISPARITY_MAX=80;
QUALITY_VERIFY_ENABLE=1;
QUALITY_BAYER_PATTERN=0;
QUALITY_PD_L_LOCATION=0;
QUALITY_PD_R_LOCATION=0;
QUALITY_XTALK_MAX=7;
QUALITY_XTALK_OB=64;
QUALITY_LRDiff_L_MIN=0.4;
QUALITY_LRDiff_L_MAX=2.5;
QUALITY_LRDiff_C_MIN=0.66;
QUALITY_LRDiff_C_MAX=1.5;
QUALITY_LRDiff_R_MIN=0.4;
QUALITY_LRDiff_R_MAX=2.5;
QUALITY_SENSITIVITY_MIN=0.45;
QUALITY_DEFECT_LEVEL=15;
*/
static SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
    ...
    .i4PitchY  = 64,
    .i4PairNum  =16,
    .i4SubBlkW  =16,//PD_DENSITY_X
    .i4SubBlkH  =16,//PD_DENSITY_Y
...
extern bool read_3l9_pdaf_data( kal_uint16 addr, BYTE* data, kal_uint32 size);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_PDAF_DATA:
			read_3l9_pdaf_data((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
