...
#include "imx214ofilmmipiraw_Sensor.h"

#include <linux/kthread.h>

#define PFX "IMX214_ofilm_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
#define IMX214_OFILM_OTP

static DEFINE_SPINLOCK(imgsensor_drv_lock);

#define OTP_SIZE 964
u8 Ofilm_imx214_OTPData[OTP_SIZE];
extern  int iReadData_Ofilm(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata);
			...
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				if(IMX214_Read_module_id_ofilm() == 0x07)
				{
					...
					*sensor_id = IMX214_OFILM_SENSOR_ID;
				}
				else
				{
...
static uint32_t RG_Ratio_typical_Ofilm=0x0239;
static uint32_t BG_Ratio_typical_Ofilm=0x02CE;
...
// return 0
static int imx214_update_awb_gain_ofilm()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
...
static int imx214_update_otp_wb_ofilm(void)
{
	...
	if(IMX214_Read_module_id_ofilm() == 0x07)
	{
		...
		iReadData_Ofilm(0x09,4,awb_otp_data);//o-film
	...
	#if IMX214_OFILM_AWB_OTP
	imx214_update_awb_gain_ofilm();  
	#endif
...
extern void	SetH1cMod_OFILM( unsigned char	UcSetNum_OFILM );
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    SetH1cMod_OFILM(0xff);  //movie mode
