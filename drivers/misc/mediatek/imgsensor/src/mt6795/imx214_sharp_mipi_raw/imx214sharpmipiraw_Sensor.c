...
#include "imx214sharpmipiraw_Sensor.h"

#define PFX "IMX214_sharp_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
#define IMX214_SHARP_OTP
...
#define OTP_SIZE 964
u8 Sharp_imx214_OTPData[OTP_SIZE];
extern  int iReadData_Sharp(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata);
extern int boot_read_sharp_2a_otp(void);

extern kal_uint8 IMX214_Read_module_id_sharp(void);
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x465B,0x2C);
    write_cmos_sensor(0x465F,0x2B);
    write_cmos_sensor(0x4663,0x2B);
    write_cmos_sensor(0x4667,0x24);
    write_cmos_sensor(0x466F,0x24);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				if(IMX214_Read_module_id_sharp() == 0x01)
				{
					Main_Camera_MID = 2;// 
					*sensor_id = IMX214_SHARP_SENSOR_ID;
				}
				else
				{
				...
				iReadData_Sharp(0x00,964,Sharp_imx214_OTPData);
				boot_read_sharp_2a_otp();
				return ERROR_NONE;
			}	
...
static uint32_t RG_Ratio_typical_Sharp=0x0101;//Use Golden module OTP value
static uint32_t BG_Ratio_typical_Sharp=0x012A;

static uint32_t GAIN_DEFAULT=0x100;
static uint32_t R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN, G_GAIN;

// R_gain: red gain of sensor AWB, 0x400 = 1
// G_gain: green gain of sensor AWB, 0x400 = 1
// B_gain: blue gain of sensor AWB, 0x400 = 1
// return 0
static int imx214_update_awb_gain_sharp()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
...
static int imx214_update_otp_wb_sharp(void)
{
	...
	if(IMX214_Read_module_id_sharp() == 0x01)
	{
        ...
        BG_Ratio_typical = BG_Ratio_typical_Sharp;

		iReadData_Sharp(0x01,4,awb_otp_data);//sharp
    ...
    //LOG_INF(" G_gain == 0x%x\n",G_GAIN);
    //LOG_INF(" B_gain == 0x%x\n",B_GAIN);
	#ifdef IMX214_SHARP_OTP_AWB
	imx214_update_awb_gain_sharp();  
	#endif
...
extern int LC898122AF_Ois_Init_Thread(void *unused);
extern struct task_struct  *ois_thread;
...
extern int  g_s4LC898122AF_Inited;
