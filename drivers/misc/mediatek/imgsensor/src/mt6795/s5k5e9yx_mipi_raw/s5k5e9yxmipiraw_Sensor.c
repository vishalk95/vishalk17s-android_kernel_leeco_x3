...
struct s5k5e9_otp_struct current_5e9_otp;
...
**************************************************************************************************/
bool s5k5e9_start_read_otp(u8 zone)
{
	u8 val = 0;
	int i;
    LOG_INF("s5k5e9_start_read_otp page = 0x%02x.\n",zone);
...
/*************************************************************************************************
* Function    :  s5k5e9_check_otp_group_flag
* Description :  before read otp , set the reading block setting  
* Parameters  :  [BYTE] zone : OTP PAGE index , 0x00~0x0f
* Return      :  0, reading block setting err
                 1, reading block setting ok 
**************************************************************************************************/
bool s5k5e9_check_otp_group_flag(u8 group)
{
	u8 val = 0;
	int i;
    LOG_INF("s5k5e9_check_otp_group group = 0x%02x.\n",group);
	
	s5k5e9_start_read_otp(4);

	if(group == 2)
	{
		val = read_cmos_sensor(0x0A33);
	    LOG_INF("s5k5e9 otp group 2 flag = 0x%02x.\n",val);
	}
	else if(group == 1)
	{
		val = read_cmos_sensor(0x0A1B);
	    LOG_INF("s5k5e9 otp group 1 flag = 0x%02x.\n",val);
	}
...
int s5k5e9_read_otp_wb(struct s5k5e9_otp_struct *otp)
{
	u8 group2_flag = 0;	
	u8 group1_flag = 0;	
	u8 group_id = 0;	
	...
	group2_flag = s5k5e9_check_otp_group_flag(2);
	...
	{
		group1_flag = s5k5e9_check_otp_group_flag(1);
    ...
    LOG_INF("s5k5e9 otp group_id = 0x%02x.\n",group_id);
...
void s5k5e9_algorithm_otp_wb1(struct s5k5e9_otp_struct *otp)
{
...
void s5k5e9_write_otp_wb(struct s5k5e9_otp_struct *otp)
{
...
void s5k5e9_read_static_register_from_otp(void)
{
	memset(&current_5e9_otp, 0, sizeof(struct s5k5e9_otp_struct));
	
    s5k5e9_read_otp_wb(&current_5e9_otp);
    s5k5e9_algorithm_otp_wb1(&current_5e9_otp);
}

int s5k5e9_get_module_id(void)
{	
	int mid = 0;
	
    mid = current_5e9_otp.mid;
	
	return mid;
}

/*************************************************************************************************
* Function    :  s5k5e9_get_otp_LSC_flag
* Description :  get otp LSC_WRITTEN_FLAG  
* Parameters  :  [BYTE] zone : OTP PAGE index , 0x00~0x0f
* Return      :  [BYTE], if 0x00 , this type has valid or empty otp data, otherwise, invalid otp data
**************************************************************************************************/
static BYTE s5k5e9_get_otp_LSC_flag(BYTE zone)
{
    BYTE flag_LSC = 0x00;
	
	s5k5e9_start_read_otp(zone);
...
**************************************************************************************************/
static bool s5k5e9_otp_lenc_update(void)
{
    BYTE  flag_LSC_page = 0;
    unsigned int  flag_LSC = 0;
    BYTE  temp = 0;
    flag_LSC_page = s5k5e9_get_otp_LSC_flag(10);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {	
				...
				#ifdef S5K5E9_OTP_ENABLE
				s5k5e9_read_static_register_from_otp();
				if(s5k5e9_get_module_id()!=7)
				{
					LOG_INF("Sunny 5E8 module!!!!\n");
					Sub_Camera_MID = 1;// 
...
static kal_uint32 open(void)
{
	...
	#ifdef S5K5E9_OTP_ENABLE
	s5k5e9_write_otp_wb(&current_5e9_otp);
	s5k5e9_otp_lenc_update();
	#endif
