...
static int ltr579_dynamic_calibrate(void)
{
	...
	printk(" ltr579_dynamic_calibrate end:noise=%d, obj->ps_thd_val_low= %d , \
		obj->ps_thd_val_high = %d\n", noise, ps_thd_val_low, ps_thd_val_high);
...
static int ltr579_als_read(int gainrange)
{
    ...
    luxdata_int /= 1000;
#endif 
	APS_DBG("Alvin:als_value_lux =%d multiply=%d", luxdata_int,als_cali_multiply_num);
	return luxdata_int;
...
int ltr579_setup_eint(struct i2c_client *client)
{
	...
	if (boot_mode == FACTORY_BOOT) {
		mt_eint_mask(CUST_EINT_ALS_NUM);
		printk("ltr579_setup_eint: ftm mode mask INT\n");
	}else{
		mt_eint_unmask(CUST_EINT_ALS_NUM);  
	}
	...
	switch (command)
	{
			...
			else
			{
				...
				else
				{
					if(!test_bit(CMC_BIT_PS,  &obj->enable))
					{
						...
						printk("ltr579_als_operate: psensor working do nothing");
					}
				}
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				sensor_data->values[0] = ((obj->als*als_test2real_rate1)/als_test2real_rate2);
				#else
				sensor_data->values[0] = ltr579_get_als_value(obj, obj->als);
				#endif
				sensor_data->values[1] = 1000;
				printk("ltr579_als_operate: values[0]=%d\n",sensor_data->values[0]);
...
static int ltr579_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = ltr579_create_attr(&ltr579_alsps_driver.driver))
	{
...
static int __init ltr579_init(void)
{
	...
	if(platform_driver_register(&ltr579_alsps_driver))
	{
