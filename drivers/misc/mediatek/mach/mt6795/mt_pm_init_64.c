...
#ifdef CONFIG_X3_HWID
static char *hwid_string[] = {"EVT", "DVT1", "DVT2", "PVT"} ;
static unsigned revision_gpio[] = {53,54};
#else
static char *hwid_string[] = {"EVT", "DVT", "PVT"} ;

static unsigned revision_gpio[] = {84,85};
#endif
...
static const char *prcb_hwid_proc_read(void)
{
    int i, len = 0 ;
    int hwid = -1 ;

	hwid = target_get_board_hwid_no() ;
	
	for(i = 0; i < ARRAY_SIZE(hwid_string); i++)
	{
		if(hwid >= 0 && hwid < ARRAY_SIZE(hwid_string))
			return hwid_string[hwid];
	}
}
#ifdef CONFIG_X3_HWID
static const char *prcb_rf_proc_read(void)
{

	if(mt_get_gpio_in(194) > 0)
		return "APEU/WW";
	else
		return "CU/CMCC";
}
static const char *prcb_charge_proc_read(void)
{
	if(mt_get_gpio_in(52) > 0)
		return "CHARGE L";
	else
		return "CHARGE H";
}
#endif
static int hwid_speed_dump_read(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", prcb_hwid_proc_read());
    return 0;
}
...
static int rf_speed_dump_read(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", prcb_rf_proc_read());
    return 0;
}
static int charge_speed_dump_read(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", prcb_charge_proc_read());
    return 0;
}
#endif
static int proc_hwid_open(struct inode *inode, struct file *file)
{
    return single_open(file, hwid_speed_dump_read, NULL);
}
#ifdef CONFIG_X3_HWID
static int proc_rf_open(struct inode *inode, struct file *file)
{
    return single_open(file, rf_speed_dump_read, NULL);
}

static int proc_charge_open(struct inode *inode, struct file *file)
{
    return single_open(file, charge_speed_dump_read, NULL);
}
#endif
...
#endif
		entry = proc_create("cust_project", S_IRUGO, NULL, &hwid_fops);
#ifdef CONFIG_X3_HWID
		entry = proc_create("rf_set_status", S_IRUGO, NULL, &rf_fops);
		entry = proc_create("charge_set_status", S_IRUGO, NULL, &charge_fops);
#endif
//FIXME
#if 0
