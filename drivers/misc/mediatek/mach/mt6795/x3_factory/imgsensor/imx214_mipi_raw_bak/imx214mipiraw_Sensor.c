...
extern int iReadReg_8(u8 a_u2Addr , u8 *a_puBuff , u16 i2cId);
...
static kal_uint16 read_sharp_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
      iReadReg((u8) addr ,(u8*)&get_byte, 0xA0);
      return get_byte;
}
static kal_uint16 read_ofilm_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
      iReadReg((u8) addr ,(u8*)&get_byte, 0xA6);
      return get_byte;
}
...
kal_uint8 IMX214_Read_module_id(void)
{	
	u8 readbuff = 0;
	u8 readbuff_RG = 0;
	u8 readbuff_BG = 0;

	LOG_INF("IMX214_Read_module_id  !!\n");

	readbuff = read_sharp_cmos_sensor(0x80);
	readbuff_RG = read_sharp_cmos_sensor(0x05);
	readbuff_BG = read_sharp_cmos_sensor(0x06);
	
	if(readbuff == 0x01)
	{
		if((readbuff_RG != 0)&&(readbuff_BG != 0))
		{
			LOG_INF("Sharp IMX214\n");
			return 1;
		}
		else
		{
			readbuff = read_ofilm_cmos_sensor(0xDB);
			
			if(readbuff == 0x07)
			{
				LOG_INF("Ofilm IMX214 strange\n");
				return 7;
			}
		}
	}
