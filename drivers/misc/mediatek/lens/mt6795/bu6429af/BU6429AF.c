/*
 * BU6429 voice coil motor driver
 *
 *
 */
...
static int init_setting(void)
{
	...
	MSB = (char)((PS << 7) | (EN << 6) | (Mode << 2)); //HF 2015/10/12

	//convert frequency to register setting
	if (resonant_frequency > 255) resonant_frequency = 255;
	...
	if(Xmode== 0)
	{
		...
		for(i = 0; i < 21; i++)
		{
			...
			if (i4RetValue < 0) 
			{
				LOG_INF("[BU6429AF] I2C send Q fact %d failed!! \n", i);
				return -1;
			}
		}
	...
	//step 5: Move the lens to center (512DAC)
	puSendCmd[0] =(char)(0xc6);
	puSendCmd[1] =(char)(0x00);
	//puSendCmd[2] ={(char)(0xc6), (char)(0x00)};
	//i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	if (i4RetValue < 0)
	{
        LOG_INF("[BU6429AF] I2C send failed!! \n");
...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    if (g_s4AF_Opened == 2)
    {
		...
		{
			if(g_s4BU6429AF_Inited == 0)
			{
				mdelay(5);
				LOG_INF("[BU6429AF] delay 5ms for  init done!\n");
			}
			else
			{
				LOG_INF("[BU6429AF]  init delay_count = %d\n", delay_count);
				break;
			}
		}
        g_s4AF_Opened = 3;
...
kal_bool BU6429_Alive(void)
{
    ...
    {
		if(s4AF_ReadReg(&InitPos) == 0)
		{
            LOG_INF("BU6429 is Alive!!! \n");
		    return TRUE;
		}
    }

    LOG_INF("BU6429 is not Alive!!! \n");
    return FALSE;
}
...
int BU6429AF_Init_Thread(void *unused)
{
    LOG_INF("Init start\n");
	init_setting();
    LOG_INF("Init end\n");
	g_s4BU6429AF_Inited = 1;
	 
    return 0;
}

struct task_struct  *Ofilm_bu6429af_init_thread;
