...
unsigned char	TneGvc( void )
{
	...
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGxoVal = (unsigned short)GenMes( AD2Z, 0 );		// 64	GYRMON1(0x1110) <- GXADZ(0x144A)
	...
	RegWriteA( WC_MES1ADD1, 0x00 ) ;		// 0x0195
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGyoVal = (unsigned short)GenMes( AD3Z, 0 );		// 64	GYRMON2(0x1111) <- GYADZ(0x14CA)
