    ...
    switch(cmd)
    {
    	...
    	case FLASH_IOC_GET_FLASH_DUTY:
			{			
				...
				{
					logE("[FLASH_IOC_IS_CHARGER_IN] function pointer is wrong ~");
				}
			}
    	    break;
    	...
    	case FLASH_IOC_GET_FLASH_ONOFF_STATUS:
			{
				...
				if(pF!=0)
				{
		            ...
		            if(copy_to_user((void __user *) arg , (void*)&kdArg , sizeof(kdStrobeDrvArg)))
					{
						logE("[FLASH_IOC_GET_FLASH_ONOFF_STATUS] ioctl copy to user failed ~");
						return -EFAULT;
					}
				}
				else
				{
					logE("[FLASH_IOC_GET_FLASH_ONOFF_STATUS] function pointer is wrong ~");
				}
			}
