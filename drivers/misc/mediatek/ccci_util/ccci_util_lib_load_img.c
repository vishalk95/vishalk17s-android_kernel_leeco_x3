...
static DEFINE_MUTEX(secu_init_mutex); //add here-mtk 20151221
...
static int signature_check_v2(int md_id, char* file_path, unsigned int *sec_tail_length)
{
	...
	if( masp_ccci_signfmt_verify_file(file_path, &bypass_sec_header_offset, &sec_total_len) == 0 ){
		...
		//return -CCCI_ERR_LOAD_IMG_SIGN_FAIL;
		CCCI_UTIL_INF_MSG_WITH_ID(md_id, "bypass sign check!, letv workground\n");
...
int ccci_init_security(void)
{
	...
	mutex_lock(&secu_init_mutex); //add here-mtk 20151221
	if(security_init)
	//	return ret;
		goto  out; //change here-mtk 20151221
	//security_init = 1; //mark here-mtk 20151221
	
	if ((ret = masp_boot_init()) !=0) {
		CCCI_UTIL_ERR_MSG("masp_boot_init fail: %d\n", ret);
		ret= -EIO;
		goto  out; //add here-mtk 20151221
	}
	...
	security_init = 1; //add here-mtk 20151221
out: //add here-mtk 20151221
	CCCI_UTIL_INF_MSG("security is on!\n");
	mutex_unlock(&secu_init_mutex); //add here-mtk 20151221
#else
