...
typedef enum {
	USB_ADC_TYPE_UNKNOWN_CABLE  = 0,   // unknown cable insert
    USB_ADC_TYPE_UNSUPPORT_CABLE = 1,  //1//1K ADC Resistance
    USB_ADC_TYPE_OTG            = 2,    //5.1k or 0k resistance
    USB_ADC_TYPE_MAX            = 3,
} USB_ADC_TYPE;

#define USB_CHANNEL 1;// usb id adc detect using AUXIN1    PS:EVT3 use AUXIN0

static INT32 usb_id_volt_to_resistance(UINT32 dwVolt)
{
    INT32 ADCRes=0;
    INT32 PullUpR = 15000; //Pull up resistance is 15k
    INT32 ADCMaxVol= 1450; // adc max vol arrage

   if(dwVolt>=ADCMaxVol)
	   	ADCRes= (ADCMaxVol*PullUpR)/(1800-ADCMaxVol);
	else
		ADCRes= (dwVolt*PullUpR)/(1800-dwVolt); 

    return ADCRes;
}

static USB_ADC_TYPE usb_id_judge_usb_type_by_res(INT32 Resis)
{
	USB_ADC_TYPE type_ret= USB_ADC_TYPE_UNKNOWN_CABLE;
	INT32 Resis_0k = 100; 
	INT32 Resis_1k = 1000; 
	INT32 Resis_5_1k = 5100; 
	 
    if(Resis>=0 && Resis<=Resis_0k)
		type_ret=USB_ADC_TYPE_OTG; //judege resistance is 0
    else if(Resis>=(Resis_1k-Resis_1k/10) && Resis<=(Resis_1k+Resis_1k/10))
		type_ret=USB_ADC_TYPE_UNSUPPORT_CABLE; //judege resistance is 1k
	else if(Resis>=(Resis_5_1k-Resis_5_1k/10) && Resis<=(Resis_5_1k+Resis_5_1k/10))
		type_ret=USB_ADC_TYPE_OTG; //judege resistance is 5.1k
	else
		type_ret=USB_ADC_TYPE_UNKNOWN_CABLE; //other resistance value
		
    return type_ret;
}
extern int CC_DECODER_TYPE;

static USB_ADC_TYPE get_usb_id_adc_type(void)
{
	USB_ADC_TYPE adc_ret= USB_ADC_TYPE_UNKNOWN_CABLE;
	INT32 adc_resis=0;
	int ret = 0, data[4], i, ret_value = 0, ret_temp = 0;
	int times=5, Channel=USB_CHANNEL;// usb id adc detect using AUXIN1    
	...
	while (i--)
	{
		...
		ret += ret_temp;
		mtk_xhci_mtk_log("get_usb_id_adc_volt: ret_temp=%d\n",ret_temp);
	}

    ret = ret/times;
	ret = ret*1500/4096;
	//ret = ret*1800/4096;   // 1.8v > adc detect arrage ??
	mtk_xhci_mtk_log("usb id volt mV = %d\n",ret);

	// (2)  convert adc voltage -> resistance
	adc_resis = usb_id_volt_to_resistance(ret);
	mtk_xhci_mtk_log("usb id adc resistance= %d\n",adc_resis);

	// (3) which usb adc type
	adc_ret=usb_id_judge_usb_type_by_res(adc_resis);
	mtk_xhci_mtk_log("usb id adc type= %d\n",adc_ret);
...
void mtk_xhci_mode_switch(struct work_struct *work)
{
	...
	int retry_time=3;
	int ret = 0;
	USB_ADC_TYPE usb_adc_type= USB_ADC_TYPE_UNKNOWN_CABLE;
...
#else
       if(mtk_is_charger_4_vol())
		   	mtk_idpin_cur_stat = IDPIN_IN_DEVICE;
       else {
			usb_adc_type=get_usb_id_adc_type();
			
		    if(usb_adc_type==USB_ADC_TYPE_OTG)
				mtk_idpin_cur_stat = IDPIN_IN_HOST;
			else if(usb_adc_type==USB_ADC_TYPE_UNSUPPORT_CABLE){
				mtk_idpin_cur_stat = IDPIN_IN_DEVICE;
				//unsupport usb cable(1k) is insert,should notice uplayer and show warning UI
				mtk_xhci_mtk_log("unsupport usb cable(1k) be detected !!!\n");
			}
			else if(usb_adc_type==USB_ADC_TYPE_UNKNOWN_CABLE)
				mtk_idpin_cur_stat = IDPIN_IN_DEVICE;
       	}
#endif
