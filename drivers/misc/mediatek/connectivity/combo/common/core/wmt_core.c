...
/* iRet = (*kal_stp_tx)((PUINT8)&rWmtPktCmd, u4WmtCmdPduLen, &u4WrittenSize); */
		...
		if (iRet) {
			...
			rWmtPktCmd.aucParam[0] = (UINT8)WMTDRV_TYPE_WMT;
			u4WmtEventPduLen = WMT_HDR_LEN + 3;	/* (7) */
			WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd 0x%x send\n", rWmtPktCmd.aucParam[0]);
		...
		if (rWmtPktCmd.aucParam[0] == WMTDRV_TYPE_WMT) {
			if (0 != rWmtPktEvent.aucParam[0]) {
				WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd, 0 != status(%d)\n",
						rWmtPktEvent.aucParam[0]);
				break;
			...
			if (WMTDRV_TYPE_WMT != rWmtPktEvent.aucParam[1]) {
				WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd, %d != (%d)\n",
						WMTDRV_TYPE_WMT, rWmtPktEvent.aucParam[1]);
				break;
			}

			if (WMTDRV_TYPE_BT <= type && type < WMTDRV_TYPE_WMT) {
				switch (type) {
					case WMTDRV_TYPE_BT:
						fw_type = 2;
						break;
				...
				if (rWmtPktCmd.aucParam[1] == WMT_FUNC_CTRL_ON) {
					if (!((1 << fw_type) & rWmtPktEvent.aucParam[2])) {
						WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd, func(%d) on fail status(0x%02x)\n",
								type, rWmtPktEvent.aucParam[2]);
						ctrlPa1 = type;
						ctrlPa2 = 32;
						wmt_core_set_coredump_state(DRV_STS_FUNC_ON);
						mtk_wcn_stp_dbg_dump_package();
					...
					if (((1 << fw_type) & rWmtPktEvent.aucParam[2])) {
						WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd, func(%d) off fail status(0x%02x)\n",
								type, rWmtPktEvent.aucParam[2]);
						ctrlPa1 = type;
						ctrlPa2 = 32;
						wmt_core_set_coredump_state(DRV_STS_FUNC_ON);
				...
				WMT_ERR_FUNC("WMT-CORE: wmt_func_ctrl_cmd, func(%d) ctrl ok status(0x%02x)\n",
						type, rWmtPktEvent.aucParam[2]);
			}
		}
