...
nicTxIsTXDTemplateAllowd(IN P_ADAPTER_T prAdapter, 
    ...
    if(prMsduInfo->fgIsTXDTemplateValid && (prMsduInfo->ucControlFlag == 0) 
		...
		if (prMsduInfo->fgIsTXDTemplateValid && !prStaRec->aprTxDescTemplate[prMsduInfo->ucUserPriority])
			DBGLOG(TX, ERROR, ("TXDTemplate is valid but pointer is error, UserPriority is 0x%x\n",
