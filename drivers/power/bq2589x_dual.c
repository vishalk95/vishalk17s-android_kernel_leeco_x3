...
static int temp_current_table[TEMP_CURRENT_TABLE_LENGTH][5] = {
	{30000, 450000, 1, LEVEL0_LIMIT, LEVEL0_LIMIT},
	{33000, 400000, 1, LEVEL0_LIMIT, LEVEL0_LIMIT},
	{36000, 350000, 1, LEVEL0_LIMIT, LEVEL0_LIMIT},
	{40000, 300000, 1, LEVEL1_LIMIT, LEVEL1_LIMIT},
	{44000, 200000, 0, LEVEL2_LIMIT, LEVEL2_LIMIT},
...
static void temp_adjust_current_limit_init(int current_limit)
{
	...
	for(temp_level = 0; temp_level < TEMP_CURRENT_TABLE_LENGTH; temp_level++) {
		...
		switch(temp_current_table[temp_level][4])
		{
			case LEVEL0_LIMIT:
				if(temp_current_table[temp_level][4] >= current_limit)
				{
					temp_current_table[temp_level][3] = current_limit;
				}
				else
				{
					temp_current_table[temp_level][3] = LEVEL0_LIMIT;
				}
				break;
			case LEVEL1_LIMIT:
				if(temp_current_table[temp_level][4] >= current_limit)
				...
				{
					temp_current_table[temp_level][3] = LEVEL1_LIMIT;
				}
				break;
			case LEVEL2_LIMIT:
			default:
				...
				{
					temp_current_table[temp_level][3] = LEVEL2_LIMIT;
				}
				break;
		}
		battery_xlog_printk(BAT_LOG_CRTI,"[battery]%s:temp=%d,adjust limit=%d, current_limit:%d\n", 
				__func__, temp_level,temp_current_table[temp_level][3], current_limit);
...
static void temp_adjust_current_limit_dump(void)
{
	...
	for(temp_level = 0; temp_level < TEMP_CURRENT_TABLE_LENGTH; temp_level++)
	{
		battery_xlog_printk(BAT_LOG_CRTI,"[battery]%s:temp=%d,adjust limit=%d\n", 
				__func__, temp_level,temp_current_table[temp_level][3]);
...
static int bq2589x_adjust_charge_current(int temp)
{
	...
	if(temp_level >= last_temp_level)
	{
		...
		if(temp <= temp_current_table[last_temp_level-2][0])
		{
			last_temp_level = temp_level;
		}
		temp_level = last_temp_level-1;
	}
	...
	if(battery_cap >= 90 || screen_on)
	{
		hvdcp_enable = 0;

#ifdef LEECO_INDIA_CHARGE_LIMIT
		if(jeita_input_limit > 100000)
			jeita_input_limit = 100000;
#else
		if(jeita_input_limit > 150000)
		...
		{
#ifdef LEECO_INDIA_CHARGE_LIMIT
			jeita_input_limit = 80000;
#endif
			charge_current = 55000;
		}
	}
