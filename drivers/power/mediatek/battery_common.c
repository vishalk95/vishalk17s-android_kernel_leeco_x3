...
static kal_bool mt_battery_100Percent_tracking_check(kal_uint32 bat_vol)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[100percent], BMT_status.bat_full = %d, BMT_status.UI_SOC = %d\n",
			BMT_status.bat_full, BMT_status.UI_SOC);
	if (BMT_status.bat_full == KAL_TRUE)	/* charging full first, UI tracking to 100% */
	{
		if (BMT_status.UI_SOC >= 100) {
			...
			if(bat_vol < 4250){
				...
				if(power_check > 6) {
					...
					battery_xlog_printk(BAT_LOG_CRTI, "[100percent], power_check = %d\n", power_check);
					power_check = 0;
				}
			}
			else {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[100percent], bat_vol = %d,"
				"BMT_status.bat_charging_state = %x, BMT_status.bat_full = %d\n",
				bat_vol, BMT_status.bat_charging_state, BMT_status.bat_full);
...
static void mt_battery_Sync_UI_Percentage_to_Real(kal_uint32 bat_vol)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[Sync_before] UI_SOC=%d, SOC=%d, bat_vol = %d, BMT_status.bat_full = %d\n",
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
			if((BMT_status.UI_SOC == 100) && (bat_vol < 4300) && (BMT_status.bat_full == KAL_FALSE))
				BMT_status.UI_SOC = 99;
		}
	}
...
kal_int32 bat_get_ui_percentage(void)
{
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI,
			    ...
			    BatteryId_status.adcChannel, BatteryId_status.adcChannelVol, BatteryId_status.batteryId);
...
static ssize_t store_disable_charger(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] store_disable_charger-->disable_charger_value:%d\n",disable_charger_value);
	...
	if(buf != NULL && size != 0)
	{
	    if(disable_charger_value == 1) /*  Disable charger */
	    {
	        ...
	        battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);	
	        battery_xlog_printk(BAT_LOG_CRTI, "[Battery] store_disable_charger-->pchr_turn_off_charging-->disable_charger_value:%d\n",disable_charger_value);
            }
            ...
            if(disable_charger_value == 0) /*  Enable charger */
            {
                ...
                battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
		  battery_xlog_printk(BAT_LOG_CRTI, "[Battery] store_disable_charger-->pchr_turn_on_charging-->disable_charger_value:%d\n",disable_charger_value);
            }		
	}
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] store_disable_charger-->end\n");
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] show_BN_TestMode : %d\n", usb_charging_current);
	return sprintf(buf, "%u\n", usb_charging_current);
}
				 ...
				 size_t size)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "usb charging= %d\n",usb_charging_current);    
    return size;
}
