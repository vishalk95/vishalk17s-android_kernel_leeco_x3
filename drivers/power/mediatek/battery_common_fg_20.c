...
kal_int32 bat_get_ui_percentage(void)//berfore GM2.0 start UI_SOC2 =-100;
{
...
static void battery_timer_resume(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "[battery_timer_resume] is_pcm_timer_trigger %d bat_spm_timeout %d\n", is_pcm_timer_trigger, bat_spm_timeout);
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "[Battery] show_batter_voltage : %x\n", BMT_status.bat_vol);
	return sprintf(buf, "%u\n", BMT_status.bat_vol);
}
