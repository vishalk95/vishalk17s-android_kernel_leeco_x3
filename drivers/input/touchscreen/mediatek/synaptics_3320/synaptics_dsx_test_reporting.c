		...
		struct device_attribute *attr, char *buf)
{
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
            ...
            if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "s3320")){ //auo
			...
			else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "LL96_01")){ //ofilm
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "s3320")){
		cnt = snprintf(buf, PAGE_SIZE - count, "max = %d, min = %d, average = %d\n",
				max, min, sum/(tx_num*rx_num -10));
		buf += cnt;
		count += cnt;
	}
	else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "LL96_01")){
		cnt = snprintf(buf, PAGE_SIZE - count, "max = %d, min = %d, average = %d\n",
				max, min, sum/(tx_num*rx_num -10));
		buf += cnt;
		count += cnt;
