...
unsigned char truly_module=0;
unsigned char g_log_enable = 0;
unsigned char g_TouchPoint = 0;
...
#endif
static unsigned int shipping_customer_configuration_id_auo = 0x4005703;
static unsigned int shipping_customer_configuration_id_truly = 0x3005701;
...
{
    struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    if(!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "s3320") ||!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "X3-04-01")){
        if(rmi4_data->config_id == shipping_customer_configuration_id_auo)
            return sprintf(buf, "Pass\n");
        else
            return sprintf(buf,"TP FW check FAIL,\nshipping FW = 0x%x, \nlocal FW = 0x%x\n", 
                    shipping_customer_configuration_id_auo, rmi4_data->config_id);
    }
    else if(!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||!strcmp(rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01")){
...
#else
	msleep(200);  // this time value could adjust later
...
{
	if(!strcmp(mtkfb_find_lcm_driver(),"nt36760_dsi_vdo_sgmicro_sgm3804") ||
			!strcmp(mtkfb_find_lcm_driver(),"r69006_dsi_cmd_truly_sgm3804") ||
			!strcmp(mtkfb_find_lcm_driver(),"boe_nt36760_dsi_cmd_sgm3804"))
	{
		if(!strcmp(mtkfb_find_lcm_driver(),"r69006_dsi_cmd_truly_sgm3804"))
			truly_module=1;
