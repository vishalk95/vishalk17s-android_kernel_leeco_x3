...
#if 0    //tuly tp calibration for gesture wakeup
extern void (*truly_lcm_suspend)(void);
		...
		struct device_attribute *attr, char *buf)
{
    ...
    else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "s3320") ||
        !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-04-01"))   //auo
    {
        sprintf(report_type, "%d", F54_FULL_RAW_CAP_RX_COUPLING_COMP);
    }
    else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-05-01"))   //boe
    {
        sprintf(report_type, "%d", F54_FULL_RAW_CAP_RX_COUPLING_COMP);
    }
    else
		...
		struct device_attribute *attr, char *buf)
{
            ...
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //tuly
    {
        ...
        sprintf(report_type, "%d", F54_AMP_RAW_ADC);
            ...
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-04-01"))   //auo
    {
        spec_data_16 = open_circuit_threshold_auo;
        sprintf(report_type, "%d", F54_FULL_RAW_CAP_RX_COUPLING_COMP);
    }
    else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-05-01"))   //boe
    {
        spec_data_16 = open_circuit_threshold_boe;
        sprintf(report_type, "%d", F54_FULL_RAW_CAP_RX_COUPLING_COMP);
    }
    else
    {
    ...
    for (ii = 0; ii < tx_num; ii++) {
        for (jj = 0; jj < rx_num; jj++) {
            ...
            if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
                    !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //tuly
            {
                if(*report_data_16 > *spec_data_16 * (100 + 25)/100 || *report_data_16 < *spec_data_16 * (100 - 25)/100)
                    cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "Open test Tx:%d,Rx:%d,raw:%d,spec:%d\n",
                            ii, jj, *report_data_16, *spec_data_16);
            }
            else if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "s3320") ||
                    !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-04-01"))   //auo
            {
		...
		struct device_attribute *attr, char *buf)
{
            ...
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //auo
    {
        ...
        truly_lcm_suspend();
        msleep(100);
        test_do_afe_calibration(F54_AFE_IS_CAL);
        truly_lcm_registers();
