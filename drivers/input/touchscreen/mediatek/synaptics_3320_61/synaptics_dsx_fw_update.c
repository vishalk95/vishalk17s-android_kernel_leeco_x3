...
#define FW_IMAGE_AUO "synaptics/startup_fw_auo.img"
#define FW_IMAGE_TRULY "synaptics/startup_fw_truly.img"
#define FW_IMAGE_BOE "synaptics/startup_fw_boe.img"
...
struct synaptics_rmi4_fwu_handle {
	...
	struct delayed_work customer_id_work;
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		if(!strcmp(fwu->product_id_string, "s3320") ||!strcmp(fwu->product_id_string, "X3-04-01"))
        {
            retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                    FW_IMAGE_AUO, sizeof(FW_IMAGE_AUO),
                    sizeof(FW_IMAGE_AUO));
        }
		else if(!strcmp(fwu->product_id_string, "S332U") ||!strcmp(fwu->product_id_string, "X3-03-01"))
        {
            retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                    FW_IMAGE_TRULY, sizeof(FW_IMAGE_TRULY),
                    sizeof(FW_IMAGE_TRULY));
        }
        else if(!strcmp(fwu->product_id_string, "X3-05-01"))
        {
            retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
exit:
	...
	rmi4_data->config_id = be_to_uint(fwu->config_id);
...
#ifdef DO_STARTUP_FW_UPDATE
static void read_customer_id_work(struct work_struct *work)
{
	unsigned short config_area = fwu->config_area;
		...
		retval = secure_memcpy(fwu->rmi4_data->rmi4_mod_info.product_id_string,
				sizeof(fwu->rmi4_data->rmi4_mod_info.product_id_string),
				fwu->read_config_buf,
		...
		retval = secure_memcpy(fwu->product_id_string,
					sizeof(fwu->product_id_string),
					fwu->rmi4_data->rmi4_mod_info.product_id_string,
					sizeof(fwu->rmi4_data->rmi4_mod_info.product_id_string),
					PRODUCT_ID_SIZE);
			if (retval < 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Failed to copy product ID string from rmi4_data \n",
						__func__);
			}
		fwu->product_id_string[PRODUCT_ID_SIZE] = 0;
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	INIT_DELAYED_WORK(&fwu->customer_id_work, read_customer_id_work);
	queue_delayed_work(fwu->fwu_workqueue,
			&fwu->customer_id_work,
			msecs_to_jiffies(100));
	queue_delayed_work(fwu->fwu_workqueue,
