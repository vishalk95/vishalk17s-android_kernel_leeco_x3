...
#define CALIBRATION_TIMEOUT_IS 30
...
show_prototype(icon_differ_data)
...
//amin
ssize_t (*synaptics_raw_cap_data_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_raw_cap_data_show;
ssize_t (*synaptics_icon_differ_data_show)(struct device *dev, struct device_attribute *attr, char *buf)=synaptics_rmi4_f54_icon_differ_data_show;
ssize_t (*synaptics_open_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_open_circuit_test_show;
ssize_t (*synaptics_short_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_short_circuit_test_show;
ssize_t (*synaptics_do_afe_calibration_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_do_afe_calibration_show;

ssize_t (*synaptics_calibration_data_freq0_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_calibration_data_freq0_show;

ssize_t (*synaptics_calibration_data_freq1_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_calibration_data_freq1_show;

ssize_t (*synaptics_calibration_data_freq2_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_calibration_data_freq2_show;

ssize_t (*synaptics_calibration_data_is_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_calibration_data_is_show;

ssize_t (*synaptics_calibration_data_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = synaptics_rmi4_f54_calibration_data_test_show;

#if 1    //tuly tp calibration for gesture wakeup
//extern void (*truly_lcm_sleep)(void);
//extern void (*truly_lcm_registers)(void);
extern void mtkfb_early_suspend(struct early_suspend *h);
extern void mtkfb_late_resume(struct early_suspend *h);
#endif
extern int sgm3804_read_bytes(unsigned char addr, unsigned char *value ,unsigned char target_vol);
...
static DECLARE_COMPLETION(f54_remove_complete);
...
static int do_preparation(void)
{
	...
	if (f54->report_type == F54_AMP_RAW_ADC) {
...
static int test_do_afe_calibration(enum f54_afe_cal mode)
{
	...
	unsigned char data_31;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	unsigned char crc_offset = DATA_REPORT_DATA_OFFSET;
	...
	if (f54->query.has_interference_metric)
		crc_offset+=2;
	/* data_7 */
	if (f54->query.has_one_byte_report_rate | f54->query.has_two_byte_report_rate)
		crc_offset++;
	...
	if (f54->query.has_multi_metric_state_machine)
		crc_offset++;
	/* data_10 */
	if (f54->query.has_multi_metric_state_machine | f54->query.has_noise_state)
		crc_offset++;	
	/* data_11 */
	...
	dev_err(&rmi4_data->i2c_client->dev,
			"%s: the CRC_OFFSET addr is 0x%02x\n",
			__func__, crc_offset);
	dev_err(&rmi4_data->i2c_client->dev,
			"%s: the ctrl188 addr is 0x%04x\n",
			__func__, f54->control.reg_188->address);
	dev_err(&rmi4_data->i2c_client->dev,
			"%s: f54->data_base_addr + crc_offset = 0x%x\n",
			__func__, f54->data_base_addr + crc_offset);

	switch (mode) {
	case F54_AFE_CAL:
	timeout = CALIBRATION_TIMEOUT_S;
		/* do the calibration */
		if (f54->query_49.has_ctrl188) {
			retval = f54->fn_ptr->read(rmi4_data,
					f54->control.reg_188->address,
		...
		do {
            ...
            {
                dev_err(&rmi4_data->i2c_client->dev, "%s: F54_AFE_CAL timeout, failed\n", __func__);
                return -EBUSY;
            }
		} while (timeout_count < timeout);
		/* check CRC */
		retval = f54->fn_ptr->read(rmi4_data,
				f54->data_base_addr + crc_offset,
				&data_31,
				sizeof(data_31));
        TP_DEBUG("F54_AFE_CAL data_31: 0x%x\n", data_31);
		if (data_31 & 1 << F54_AFE_CAL)
        {
            dev_err(&rmi4_data->i2c_client->dev, "%s: F54_AFE_CAL failed\n", __func__);
			return -EBUSY;
        }
		break;
	case F54_AFE_IS_CAL:
		if (f54->query_49.has_ctrl188) {
			...
			if (retval < 0) {
				dev_err(&rmi4_data->i2c_client->dev,
						"%s: Failed to start is calibration\n",
						__func__);
				return retval;
			}
		}
		do {
    timeout = CALIBRATION_TIMEOUT_IS;
			retval = f54->fn_ptr->read(rmi4_data,
					f54->control.reg_188->address,
					f54->control.reg_188->data,
					sizeof(f54->control.reg_188->data));
			...
			if (timeout_count == timeout)
            {
                dev_err(&rmi4_data->i2c_client->dev, "%s: F54_AFE_IS_CAL timeout, failed\n", __func__);
				return -EBUSY;
            }
		} while (timeout_count < timeout);
		/* check CRC */
		retval = f54->fn_ptr->read(rmi4_data,
				...
				&data_31,
				sizeof(data_31));
		if (data_31 & 1 << F54_AFE_IS_CAL)
        {
            dev_err(&rmi4_data->i2c_client->dev, "%s: F54_AFE_IS_CAL failed\n", __func__);
            return -EBUSY;
        }
		break;
	default:
...
static void free_control_mem(void)
{
	...
	printk("free_control_mem:time=%d \n",time);
    time++;

	if(!attrs_ctrl_regs_exist[0])
		return;

	printk("free_control_mem: start free! \n");
	...
	printk("control.reg_86=%p\n",control.reg_86);
	kfree(control.reg_86);
	printk("control.reg_86 freed\n");
	...
	attrs_ctrl_regs_exist[0] = false;

	printk("free_control_mem: end free! \n");
	return;
}
...
static ssize_t synaptics_rmi4_f54_read_report(struct device *dev,
...
static void synaptics_rmi4_rawdata_store(char *str, s32 len)
{
    ...
    if (IS_ERR(arch_filp))
    {
        printk("Failed to open %s for test result archive!", TEST_RSLT_ARCHIVE_PATH);
        return;
    }
    old_fs = get_fs();
    set_fs(KERNEL_DS);
...
static ssize_t synaptics_rmi4_f54_calibration_data_freq0_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	unsigned char *head;
 	...
 	if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
        	!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))
...
static ssize_t synaptics_rmi4_f54_calibration_data_freq1_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
...
static ssize_t synaptics_rmi4_f54_calibration_data_freq2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
...
static ssize_t synaptics_rmi4_f54_calibration_data_is_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
...
static ssize_t synaptics_rmi4_f54_calibration_data_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
...
/*	freq0 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
    	    	{
				if(*report_data_8  > 0x07 ||*report_data_8 < 0x02)
                    		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 image Tx:%d Rx:%d data:%x,Spec:0x02~0x07\n",
                            ii, jj, *report_data_8);
			report_data_8 +=2;
	...
	{
			if(ii == 0 || ii == 1){
				if(*report_data_8  > 0x2d ||*report_data_8 < 0x10)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x10~0x2d\n",ii,*report_data_8);
				}
			if(ii == 2){
					if(*report_data_8  > 0x17 ||*report_data_8 < 0x13)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x13~0x17\n",ii,*report_data_8);
				}
			if(ii == 3){
					if(*report_data_8  > 0x90 ||*report_data_8 < 0x50)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x50~0x90\n",ii,*report_data_8);
			}
		report_data_8 +=2;
	}
/*	freq0 test  end */
...
/*	freq1 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
      	 	{
		if(*report_data_8  > 0x07 ||*report_data_8 < 0x02)
                    		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 image Tx:%d Rx:%d data:%x,Spec:0x02~0x07\n",
                            ii, jj, *report_data_8);
			report_data_8 +=2;
	...
	for (ii = 0; ii < 4; ii++)
	{
			if(ii == 0 || ii == 1){
				if(*report_data_8  > 0x2d ||*report_data_8 < 0x10)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x10~0x2d\n",ii,*report_data_8);
				}
			if(ii == 2){
					if(*report_data_8  > 0x17 ||*report_data_8 < 0x13)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x13~0x17\n",ii,*report_data_8);
				}
			if(ii == 3){
					if(*report_data_8  > 0x90 ||*report_data_8 < 0x50)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x50~0x90\n",ii,*report_data_8);
			}
		report_data_8 +=2;
	}
/*	freq1 test  end */
...
/*	freq2 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
      	  	{
			if(*report_data_8  > 0x07 ||*report_data_8 < 0x02)
                    		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 image Tx:%d Rx:%d data:%x,Spec:0x02~0x07\n",
                            ii, jj, *report_data_8);
			report_data_8 +=2;
		}
	}
	...
	for (ii = 0; ii < 4; ii++)
	{
		if(ii == 0 || ii == 1){
				if(*report_data_8  > 0x2d ||*report_data_8 < 0x10)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x10~0x2d\n",ii,*report_data_8);
				}
			if(ii == 2){
					if(*report_data_8  > 0x17 ||*report_data_8 < 0x13)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x13~0x17\n",ii,*report_data_8);
				}
			if(ii == 3){
					if(*report_data_8  > 0x90 ||*report_data_8 < 0x50)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x50~0x90\n",ii,*report_data_8);
			}
		report_data_8 +=2;
	}
/*	freq2 test  end */
...
static ssize_t synaptics_rmi4_f54_icon_differ_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii;
    ...
    if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
        !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //tuly
    {
        sprintf(report_type, "%d", F54_16BIT_IMAGE);
    }
    else
    {
        return snprintf(buf, PAGE_SIZE, "Invalid product id:%s\n", f54->rmi4_data->rmi4_mod_info.product_id_string);
    }
	printk("synaptics read icon differ start ...\n");
	icon_differ_flag = 1;
    f54->read_button_delta = true;
    retval = synaptics_rmi4_f54_read_report(dev, attr, report_type, sizeof(report_type));
    f54->read_button_delta = false;
	icon_differ_flag = 0;
    if(retval < 0)
    {
	...
	printk("synaptics read icon differ end ...\n");
	return cnt;
}


static ssize_t synaptics_rmi4_f54_raw_cap_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
	...
	long sum = 0;
    	...
    	short min = 9999;
	char val_p, val_n;
	char val_target=0x0f; // gate driver p/n voltage!!
	int ret = 0;
...
#if 1
	ret=sgm3804_read_bytes(0x00, &val_p ,val_target);
	if(ret!=1)
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "sgm3804 val_p read fail\n");
	else
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "val_p = 0x%x target = 0x%x\n", val_p, val_target);

	ret=sgm3804_read_bytes(0x01, &val_n ,val_target);
	if(ret!=1)
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "sgm3804 val_n read fail\n");
	else
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "val_n = 0x%x target = 0x%x\n", val_n, val_target);
#endif

	return cnt;
}

static ssize_t synaptics_rmi4_f54_open_circuit_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    char report_type[10] = {0};
    char ii, jj;
    ...
    if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //tuly
    {
        spec_data_16 = open_circuit_threshold_tuly;
        sprintf(report_type, "%d", F54_AMP_RAW_ADC);
    }
    else
    {
    ...
    for (ii = 0; ii < tx_num; ii++) {
        for (jj = 0; jj < rx_num; jj++) {
            ...
            {
#if 1            
				if(ii == 30 &&( jj == 3 || jj == 4 || jj == 5 || jj == 6 || jj == 7 || jj == 8 || jj == 9 || 
					jj == 10 || jj == 11 || jj == 12 || jj == 13 || jj == 14 || jj == 15|| jj == 16|| jj == 17) )
#else
				if(ii == 30)
#endif
				{ 
					//skip empty node in Button area
				}
				else
				{
                if(*report_data_16 > 305 || *report_data_16 < 205)
                    cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "Open test Tx:%d Rx:%d Raw:%d,Spec:205~305\n",
    ...
    if(!cnt)
        ...
        report_data_16 = (short *)f54->report_data;
        count = cnt;
        //synaptics_rmi4_rawdata_store(buf, cnt);
        //sprintf(buf_tmp, "start store raw data:\n");
        synaptics_rmi4_rawdata_store(buf_tmp, strlen(buf_tmp));
        for (ii = 0; ii < tx_num; ii++) 
        {
            for (jj = 0; jj < rx_num; jj++) 
            {
        ...
        count += snprintf(buf + count, PAGE_SIZE - count, "tx = %d\trx = %d\n", tx_num, rx_num);

        count += snprintf(buf + count, PAGE_SIZE - count, "product_id:%s\n", f54->rmi4_data->rmi4_mod_info.product_id_string);

        count += snprintf(buf + count, PAGE_SIZE - count, "max = %d, min = %d, average = %ld\n", max, min, sum/(tx_num*rx_num));


        synaptics_rmi4_rawdata_store(buf, count);
#endif
        return cnt;
}
static ssize_t synaptics_rmi4_f54_short_circuit_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
    int retval;
	...
	if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //tuly do not need this    amin
    {
        return snprintf(buf, PAGE_SIZE, "Pass\n");
    }

	sprintf(report_type, "%d", F54_TX_TO_TX_SHORT);
	retval = synaptics_rmi4_f54_read_report(dev, attr, report_type, sizeof(report_type));
    if(retval < 0)
    {
        return snprintf(buf, PAGE_SIZE, "Do not have read report data\n");
	...
	for (ii = 0; ii < f54->report_size; ii++) {
		if(f54->report_data[ii]){
			return snprintf(buf, PAGE_SIZE, "Short test, Type-5, %03d: 0x%02x\n",
					ii, f54->report_data[ii]);
		}
	}
	sprintf(report_type, "%d", F54_TREX_SHORTS);
	retval = synaptics_rmi4_f54_read_report(dev, attr, report_type, sizeof(report_type));
    if(retval < 0)
    {
        return snprintf(buf, PAGE_SIZE, "Do not have read report data\n");
...
int truly_flag;
static ssize_t synaptics_rmi4_f54_do_afe_calibration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    int retval;
    ...
    if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S332U") ||
            !strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "X3-03-01"))   //auo
    {
...
static int synaptics_rmi4_f54_set_ctrl(void)
{
   ...
   printk("synaptics_rmi4_f54_set_ctrl: start!\n");
	...
	if (f54->query.has_ctrl88 == 1) {
		control->reg_88 = kzalloc(sizeof(*(control->reg_88)),
				GFP_KERNEL);
		if (!control->reg_88){
			printk("synaptics_rmi4_f54_set_ctrl: reg=88\n");
			goto exit_no_mem;
		}
	...
	if (f54->query_27.has_ctrl110) {
		control->reg_110 = kzalloc(sizeof(*(control->reg_110)),
				GFP_KERNEL);
		if (!control->reg_110){
			printk("synaptics_rmi4_f54_set_ctrl: reg=110\n");
			goto exit_no_mem;
		}
	...
	if (f54->query_38.has_ctrl149) {
		control->reg_149 = kzalloc(sizeof(*(control->reg_149)),
				GFP_KERNEL);
		if (!control->reg_149){
			printk("synaptics_rmi4_f54_set_ctrl: reg=149\n");
			goto exit_no_mem;
		}
	...
	if (f54->query_49.has_ctrl188) {
		control->reg_188 = kzalloc(sizeof(*(control->reg_188)),
				GFP_KERNEL);
		if (!control->reg_188){
			printk("synaptics_rmi4_f54_set_ctrl: reg=188\n");
			goto exit_no_mem;
		}
		control->reg_188->address = reg_addr;
		reg_addr += CONTROL_188_SIZE;
	}
	printk("synaptics_rmi4_f54_set_ctr: end\n");
	return 0;

exit_no_mem:
	printk("synaptics_rmi4_f54_set_ctr: reg_num=%d\n",reg_num);
	dev_err(&rmi4_data->i2c_client->dev,
...
static void synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	if (f54->read_button_delta) {
		f54->report_size = 6;
		report_index[0] = (unsigned char)(offset & 0x00ff);
		report_index[1] = (unsigned char)(offset >> 8);
	}
...
static void synaptics_rmi4_f54_reset(struct synaptics_rmi4_data *rmi4_data)
{
   ...
   if(f55){
	...
	if (!f54found) {
		printk("synaptics_rmi4_f54_reset: f54 not find\n");
		retval = -ENODEV;
		goto exit_free_mem;
	}
pdt_done:
...
{
	synaptics_rmi4_new_function_l(&f54_module, true);
...
{
	synaptics_rmi4_new_function_l(&f54_module, false);
